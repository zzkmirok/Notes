...
Those modules are taken from the `module list` of default `LAMMPS` module on cluster (25.03.2026):
...
After activate the correps, install the pytorch compatible with `CUDA-12.6` in a virtual env. (In principle the `PYTORCH` module is also available in the module system, which looks like depending on docker. Still not try it.)
...
- The following command should work similar as setting LIRARY, here might be redundant (?)

    ```shell
    CPPFLAGS="-I${_MY_LIBTORCH}/include -I${_MY_LIBTORCH}/include/torch/csrc/api/include/ -I${_MY_LIBTORCH}/include/torch" \\n    
    LDFLAGS="-Wl,-rpath,${_MY_LIBTORCH}/lib" \\n    
    ```
- `ltorch`, `ltorch_cpu`, and `ltorch_cuda` ensure that `PLUMED` can run on both cpu and gpu, so do the rest.
...
1. Intall `lammps-mace`
   ...
   2. create a `.cmake` file for configure the compilation related env. Put it in the `cmake/preset`
    ...
    # --- 5. 常用物理包补齐 (参考系统版) ---
    # --- 5. basic lammps package for this lammps-mace versin ---
...
1. Convert model downloaded from Huggingface to lammps supported model.
    ...
    ```shell
    python -m mace.cli.create_lammps_model ./MACE-matpes-pbe-omat-ft.model --dtype="float64" [--output]
    ```
